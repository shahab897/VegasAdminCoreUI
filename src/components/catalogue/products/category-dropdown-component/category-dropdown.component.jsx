...
const CategoryDropDown = (props) => {
  const { options, setCategoryId, defaultCategory, setSubcategories } = props;

  const handleChange = async (selected) => {
    setCategoryId(selected[0].value);
    ...
    axios
      .get(
        `https://vegasapi.phebsoft-team.com/api/getSubcategories/${selected[0].value}`,
        config
      )
      .then((response) => {
        setSubcategories(
