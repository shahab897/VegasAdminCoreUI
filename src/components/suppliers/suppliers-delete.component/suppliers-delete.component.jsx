...
function SuppliersDelete(props) {
  const { setUpdated, id } = props;
  const token_vegas =
  ...
  const handleDelete = () => {
    ...
    axios
      .delete(`https://vegasapi.phebsoft-team.com/api/suppliers/${id}`, config)
      .then(() => {
  ...
  return (
    <td>
      <CButton size="md" color="danger" onClick={() => handleDelete()}>
        <MdDelete />
      </CButton>
    </td>
  );
...
export default SuppliersDelete;
