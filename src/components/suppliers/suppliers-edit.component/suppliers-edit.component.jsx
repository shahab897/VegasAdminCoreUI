...
function SuppliersEdit() {
  ...
  const fetch_a = () => {
    ...
    axios
      .get(
        `https://vegasapi.phebsoft-team.com/api/suppliers/${id}`,
        axiosConfig
      )
      .then((result) => {
        ...
        setBrandsIds([
          ...brands,
          ...cat.brand_ids.split("|").map((el) => parseInt(el, 10)),
        ]);
        console.log(brandsIds, Ids);
      })
      .catch((error) => console.log("error", error));
  };
  ...
  useEffect(() => {
    console.log(brandsIds, "this has changed");
    if (brandsIds.length !== 0) setIsLoading(false);
  }, [brandsIds]);
